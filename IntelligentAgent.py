...
class IntelligentAgent(BaseAI):
    ...
    def search(self,grid):
        start=time.process_time()  #start timer for .2 second time limit 
        depth=2
        top=self.createTree(grid,depth,start)
        found=self.minimax(top,0,-math.inf,math.inf,start)
        while(time.process_time()-start<.18) : # continue searching until time limit is reached
            ...
            top=self.createTree(grid,depth,start)
            if top==False: # createTree was incomplete so ignore
                break
            hold=self.minimax(top,0,-math.inf,math.inf,start) # search tree for best board state within time
            if hold!=False:
                found=hold
    ...
    def minimax(self,top,turn,alpha,beta,start):
        #will run minimax, ie go to bottom,check for player and computer evals, return values and prune so forth
        #print(top)
        if (time.process_time()-start>.17):
            return False
        if turn%2==0:
            nturn=1 # swap from even to odd
            val=-math.inf # set first value based on finding max here or min next
        else:
            nturn=0
            val=math.inf
        if top.chance==True: # the node is a parent node and has possible children, some parents have no possible moves
            nturn=turn
            avg=0
        #alpha=-math.inf
        #beta=math.inf
        #fnode=top 
        if top.leaf==False: # not a leaf node then call minimax on next level changing turn
            for point in top.nextval:
                ...
                #    print(t,end=" ")
                #print()
                check=self.minimax(point,nturn,alpha,beta,start) #recursive odd even function
                #if check.data==None:
                #    check.data=self.evalfun2(check.dgrid)
                if check==False: #error catch
                    return False
                if top.chance==True:
                    if point.cval==2:
                        avg+=check.data*.9 #weight data according to probability of number spawn
                    else:
                         avg+=check.data*.1
                    ...
                    if check.data>val:
                        ...
                        if val>alpha:
                            alpha=check.data
                    if check.data>beta: 
                        top.data="cut" # player finds a move it knows the computer will select that is bad for the player or vice versa
                        top.cut=True
                        top.leaf=True
                        break
                else: #min it
            ...
            if top.chance==True:
                fnode=check
                fnode.data=avg #return weighted board state as data for found board
                return fnode
            if turn%2==0:
        ...
        return top    
    def evalfun2(self,grid): #heurestic function, uses smoothness, monotonicity and available grid spots
        val=self.tiles(grid)+2.5*self.mono(grid)+self.smooth(grid)
    ...
    def createTree(self,grid,depth,start): # creates a tree of a set depth alternating children from every possible player move and computer tile insertion
        top=node(grid)
    ...
    def createTreeHelp(self,top,turn): #will be good when we prune and only want to continue certain branches
        ...
        #Debugging methods for visualization and testing expected performance with set grid configurations 
